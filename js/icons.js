// SVG Icon library (replacing lucide-react)
const ICONS = {
  ...
  play: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="white" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="6 3 20 12 6 21 6 3"/></svg>',
  ...
  send: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="m22 2-11 11"/></svg>',
...
function icon(name, size) {
  ...
  return ICONS[name].replace(/width="24"/g, 'width="' + s + '"').replace(/height="24"/g, 'height="' + s + '"');
}

// Auto-replace all icon placeholders in the DOM after load
// Use setTimeout(0) to ensure this runs AFTER all other DOMContentLoaded handlers
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(function() {
    // Replace all data-icon spans
    document.querySelectorAll('[data-icon]').forEach(function (el) {
      var name = el.getAttribute('data-icon');
      var size = el.getAttribute('data-size') || 24;
      el.innerHTML = icon(name, size);
    });
    // Also replace any remaining ${icon(...)} text patterns in the body
    replaceIconPlaceholders(document.body);
  }, 0);
});

function replaceIconPlaceholders(root) {
  var walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null, false);
  var nodesToReplace = [];
  while (walker.nextNode()) {
    var node = walker.currentNode;
    if (node.nodeValue && node.nodeValue.indexOf('${icon(') !== -1) {
      nodesToReplace.push(node);
    }
  }
  nodesToReplace.forEach(function (node) {
    var span = document.createElement('span');
    span.innerHTML = node.nodeValue.replace(/\$\{icon\('([^']+)',\s*(\d+)\)\}/g, function (match, name, size) {
      return icon(name, parseInt(size));
    });
    node.parentNode.replaceChild(span, node);
  });
  // Also fix innerHTML of elements that contain icon placeholders in attributes or innerHTML
  document.querySelectorAll('a, button, span, div, li, p').forEach(function (el) {
    if (el.innerHTML.indexOf('${icon(') !== -1) {
      el.innerHTML = el.innerHTML.replace(/\$\{icon\('([^']+)',\s*(\d+)\)\}/g, function (match, name, size) {
        return icon(name, parseInt(size));
      });
    }
  });
